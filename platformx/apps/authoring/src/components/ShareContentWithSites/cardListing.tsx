...
import { capitalizeFirstLetter, getCurrentLang } from '../../utils/helperFunctions';
...
const CardListing = ({selectedItem, contentType, sitelist,  duplicateToSite,
  titledata, onClose}) => {
  ...
  `${gcpUrl}/${BucketName}/${defaultImage}`;
  ...
  const handleNextClick = () => {
    ...
    if(sitename && sitename !== ''){
      duplicateToSite(false, titledata, selectedItem, sitename).then((res)=>{
...
function createUrlFromTitle(title: string): string {
  ...
  const urlFriendlyTitle = lowercaseTitle?.replace(/\s+/g, '-')?.replace(/[^a-z0-9-]/g, '');
  ...
  return (
    <Box>
      ...
      <Box className={classes.container}>
        <Box className={classes.contentHeader} sx={{marginTop: {xs: '50px', md: '0px'}}}>
          ...
          <Box sx={{display: 'flex'}}>
            <Box component='span' className={classes.cancelBtn} onClick={onClose}>
              {t('cancel')}
            </Box>
            <Box component='span' className={classes.doneBtn} onClick={handleNextClick}>
              {t('done')}
            </Box>
          </Box>
        </Box>
        ...
        <Box
            className={classes.SiteShareScrollBox}
          >
          <Box className={classes.itemBoxWrap}>
          {filteredSiteList.length > 0 && filteredSiteList.map((site, index) =>
              ...
              <Box
                ...
                <Grid container>
                    ...
                    <Box className={classes.imgContainer}>
                      <img
                            className={classes.siteImg}
                             src={site.header_logo ? `${process.env.REACT_APP_GCP_URL  }/${  process.env.REACT_APP_BUCKET_NAME  }/${  site.header_logo}` : SitePlaceholder}
                            //src={SitePlaceholder}
                            onClick={() => navigate(`/sites/site-creation/${site.site_title}`)}
                          />
                    </Box>
                  </Grid>
                  <SiteNameBox item xs={12} sm={10} md={10} lg={10}>
                    ...
                    <SiteDomainTypo>
                      Domain:{' '}
                      <SiteLink href={`https://${site.site_address}.${site.domain_name.replace('.com.', '.com')}`} target="_blank">{`${site.site_address}.${site.domain_name.replace('.com.', '.com')}`}</SiteLink>
                    </SiteDomainTypo>
      ...
      {showSuccessPopup ? (
        <PlateformXDialog
            ...
            confirmButtonHandle={() =>  window.open(`${process.env.REACT_APP_BASE_URL}/${sitename}/${getCurrentLang()}/content/create-${contentType?.toLowerCase()}?path=${createUrlFromTitle(duplicateTitle)}`, '_self')}
