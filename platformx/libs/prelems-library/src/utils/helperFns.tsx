...
export const relativeImageURL = (gcpUrl: string, bucketName: string, url: string, ext: string) => {
...
export const formateNumber = (value: number, toFixedVal?: number) => {
  if (!isNaN(value)) {
    return (Math.round(value * 100) / 100).toFixed(!isNaN(toFixedVal) ? toFixedVal : 2);
  }
  return value;
};
