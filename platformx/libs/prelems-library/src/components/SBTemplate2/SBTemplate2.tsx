...
import { useCustomStyle } from "./SBTemplate2.style";
...
SBTemplate2Prop) => {
  ...
  const defaultStructureData = () => {
    ...
    return SBTemplate2StructureData;
  };
  ...
  const generateStructureData = () => {
    ...
    if (firstRender.current === true) {
      ...
      if (String(tempSD).length > 0) {
        SBTemplate2StructureData = JSON.parse(tempSD);
      } else {
        SBTemplate2StructureData = defaultStructureData();
      }
...
*/
  ...
  return (
    <div
      ...
      className={`${classes.SBTemplate2Wrapper} ${globalClasses.prelemType1} prelem prelemType1 SBTemplate2Bg`}>
      <Container
        className={
          authoringHelper?.isEditPage ? "grid_full_width prelem-py" : "grid_container prelem-py"
        }
        ref={ref}>
        <Box className='SBTemplate2'>
          <TwoColumnLayout
            firstColumnContent={firstColumnContent()}
            secondColumnContent={secondColumnContent()}
            customClassName='SBTemplate2'
            gridVal={gridVal}
            col1Align='center'
            col2Align='center'
            noGap={true}
...
SBTemplate2.defaultProps = {
  content: {
    Button1_Action: "External",
    Button1_Content:
      '{"pagination":{"start":0,"rows":10},"searchTerm":"","tags":[],"filter":"SBTemplate2","isSuggestive":false,"SBTemplate2":{"filter":[]}}',
