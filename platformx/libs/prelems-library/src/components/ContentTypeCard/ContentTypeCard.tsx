...
import { getImagebasedontagandcontenttype } from "./helper";
...
ContentTypeCardProps) => {
  const { Color = "" } = content?.background_content || {};
  const { ContentType = "" } = nullToObject(content);

  let formedImage;
  if (!isLoading && content && Object.keys(nullToObject(content)).length > 0) {
    formedImage = getImagebasedontagandcontenttype(content, secondaryArgs);
  }
