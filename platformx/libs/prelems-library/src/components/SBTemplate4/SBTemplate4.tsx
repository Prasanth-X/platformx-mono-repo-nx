...
import { useCustomStyle } from "./SBTemplate4.style";
...
SBTemplate4Prop) => {
  ...
  const defaultStructureData = () => {
    ...
    return SBTemplate4StructureData;
  };
  ...
  const generateStructureData = () => {
    ...
    if (firstRender.current === true) {
      ...
      if (String(tempSD).length > 0) {
        SBTemplate4StructureData = JSON.parse(tempSD);
      } else {
        SBTemplate4StructureData = defaultStructureData();
      }
...
*/
  ...
  return (
    <div
      ...
      className={`${classes.SBTemplate4Wrapper} ${globalClasses.prelemType1} prelem prelemType1 SBTemplate4Bg`}>
      <Container
        className={authoringHelper?.isEditPage ? "grid_full_width" : "left_grid_container"}
        ref={ref}>
        <Box className='SBTemplate4'>
          <TwoColumnLayout
            firstColumnContent={firstColumnContent()}
            secondColumnContent={secondColumnContent()}
            customClassName='SBTemplate4'
            gridVal={gridVal}
            col1Align='center'
            col2Align='center'
            noGap={true}
...
SBTemplate4.defaultProps = {
  content: {
    Button1_Action: "External",
    Button1_Content:
      '{"pagination":{"start":0,"rows":10},"searchTerm":"","tags":[],"filter":"SBTemplate4","isSuggestive":false,"SBTemplate4":{"filter":[]}}',
