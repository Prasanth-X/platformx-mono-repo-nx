...
const ServiceCard = ({ content, analytics, authoringHelper, secondaryArgs }: ServiceCardProps) => {
  ...
  const handleImageClick = async (ButtonDataObjNew?: any) => {
    ...
    switch (ButtonDataObjNew?.Button_Action) {
      case "Internal":
        if (ButtonDataObjNew?.Button_RedirectURL) {
          deployUrl = platformEndPoint + language + ButtonDataObjNew.Button_RedirectURL;
        }
        break;
      case "External":
        if (ButtonDataObjNew?.Button_RedirectURL) {
          deployUrl = completeExternalUrl(ButtonDataObjNew.Button_RedirectURL);
        }
        break;
      case "Content":
        const contentObjHandle = uriToJSON(ButtonDataObjNew);
    ...
    if (ButtonDataObjNew?.Button_Action !== "Content") {
      if (ButtonDataObjNew?.Button_Type === "current window" && deployUrl !== "") {
        window.location.href = deployUrl;
      } else {
        window.open(deployUrl, "_blank");
      }
    } else {
      const contentObjHandle = uriToJSON(ButtonDataObjNew); //convertToParse
      const { ContentType = "", currentPath = "" } = nullToObject(contentObjHandle);
      const { Button_Type: buttonType = "" } = nullToObject(ButtonDataObjNew);
                ...
                Object.entries(content?.ImageCompound).map(([key, value], index) => {
                  ...
                  return (
                    <Grid
                      ...
                      lg={4}
                      p={1}
                      className='serviceCardItems'>
                      <Box
                        onClick={() =>
                          !secondaryArgs?.editState && handleImageClick(ButtonDataObjNew)
                        }
                        sx={{
                          position: "relative",
                          cursor: "pointer",
                          "&:hover": {
                            ...
                            ".button-text": {
                              display: `${!authoringHelper?.isEditing ? "none" : ""}`,
                        ...
                        <Box className={`secondButtonWrapper image-button-text`}>
