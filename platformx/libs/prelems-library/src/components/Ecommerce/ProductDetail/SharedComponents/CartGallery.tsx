...
const CartGallery = (props: any = {}) => {
  ...
  const handleNextThumbnail = () => {
    setActiveThumbnailIndex((prevIndex) => (prevIndex === images.length - 1 ? 0 : prevIndex + 1));
  };

  const handlePrevThumbnail = () => {
    setActiveThumbnailIndex((prevIndex) => (prevIndex === 0 ? images.length - 1 : prevIndex - 1));
  };
      ...
      <Grid item className={`product-small-image-wrapper smallImageWrapper`}>
        ...
        {loading ? (
          [1, 2, 3].map((item, index) => (
            <Grid item key={convertToLowerCase(index + "initialThumbnails-ndn")}>
                ...
                <Card className={`product-thumbnail skeltonThumb`}>
                  <CardActionArea
                    ...
                    {loading ? (
                      ...
                      <CardMedia className='skeleton' component='img' image={imageUrl} />
                    )}
                  </CardActionArea>
                </Card>
              </Grid>
        ...
        {isMobile ? (
          <>
            {initialThumbnails.length > 4 && (
              <IconButton aria-label='Next' onClick={handleNextThumbnail} className='nextIcon'>
                <ChevronRight />
              </IconButton>
            )}
          </>
          ...
          <>
            {initialThumbnails.length > 5 && (
              <IconButton aria-label='Next' onClick={handleNextThumbnail} className='nextThumb'>
                <ArrowDownward />
              </IconButton>
            )}
          </>
        ...
        <Card className={`detail-large-image-card largeImageGap`}>
          <CardActionArea
            ...
            <CardMedia
              ...
              image={
                loading ? "" : images.length > 0 ? images[activeThumbnailIndex] : fallBackImage
              }
              onError={(e: any) => {
