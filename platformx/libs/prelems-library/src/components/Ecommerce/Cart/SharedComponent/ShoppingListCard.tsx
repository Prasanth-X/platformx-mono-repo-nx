...
function ShoppingListCard({
  ...
  const updateQuantityOfItem = async (quant: number) => {
    ...
    const { data: { data: { updateLineItem: { statusCode = 0, msg = "" } = {} } = {} } = {} } =
  ...
  const removeProduct = async () => {
    ...
    const { data: { data: { removeLineItem: { statusCode = 0 } = {} } = {} } = {} } =
      nullToObject(response);
  ...
  const quantityLoadingCheck =
    (quantityLoading || refetchLoading) && product?.id === iftemRefforUpdating.current;
  return (
    <>
      <ToastContainerHandle />
      {(removeItemLoading || refetchLoading) && product?.id === itemRefforRemoval.current ? (
        <ShoppingSkeletonListCard />
      ) : (
        <Box key={product?.id} className={`${classes.cartEachItemRowsWrapper} cartEachItemRows`}>
          <Box className='productRow'>
            <CloseRoundedIcon onClick={removeProduct} className='removeIconrow' />
