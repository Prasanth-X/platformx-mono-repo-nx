...
import { AreaChart, Area, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from "recharts";
...
const BigNumber = ({ itemData }: any) => {
  ...
  const getPercentageValue = () => {
    if (data[0] && data[1] && data[0][secondKey] && data[1][secondKey]) {
      const res = ((data[1][secondKey] - data[0][secondKey]) / data[0][secondKey]) * 100;
  ...
  return (
    <Box className='bigNumber pageGraph'>
      ...
      <ResponsiveContainer width={config.width} height={config.height} className='noxyAxsis'>
        <AreaChart data={data} margin={{ top: 210, right: 10, left: 10, bottom: 40 }}>
