...
export const getProductDetails = (
  ...
  const obj: String = `{pagination:{start:${start},rows:${rows}},searchTerm:${JSON.stringify(
    ...
    filter,
  )},attributes:[{key:${JSON.stringify(key)},value:${JSON.stringify(
    attributevalue,
  )}}]}}`
...
export const ecomCartIdBasedGetItem = (ele: any) => {
  ...
  return postRestApiCall(
    `${deliveryEndPoint}delivery-engine`,
    data,
    language,
    sitename,
  )
...
export const getProductDetailsApiCall = (
  ...
  const filterArray: any =
    nullToArray(filterAttr).length > 0 ? JSON.stringify(filterAttr) : []
  const {
    prelemBaseEndpoint: { deliveryEndPoint = '', language = 'en' } = {},
    sitename = '',
  } = secondaryArgs
...
export const proceedToShippingAddress = (ele: any) => {
  ...
  const addressObj = `{ title: ${JSON.stringify(
    firstName,
  )}, last_name: ${JSON.stringify(lastName)}, street_name: ${JSON.stringify(
    address,
  )}, postal_code: ${JSON.stringify(pincode)}, city: ${JSON.stringify(
    city,
  )}, state: ${JSON.stringify(
    'Alabama',
  )}, country: "US", mobile: ${JSON.stringify(
    contactNumber,
 ...
 */
export const getProductDetailsRewardpoints = (
  amt: string,
  secondaryArgs: any,
) => {
  const { prelemBaseEndpoint: { loyaltyPortalEndPoint = '' } = {} } =
    nullToObject(secondaryArgs)
  return getRestApiCall(
    `${loyaltyPortalEndPoint}v1/campaign/getCampaignPoints?amount=${amt}`,
  )
}
