export const getSelectedObject = (item: any) => {
  return {
    ...
    is_published: "yes",
    page: item?.currentPageUrl?.includes("/") ? item?.parentPageUrl?.slice(1) : item?.parentPageUrl,
    name: item?.parentPageUrl?.includes("/") ? item?.parentPageUrl?.slice(1) : item?.parentPageUrl,
    status: "published",
