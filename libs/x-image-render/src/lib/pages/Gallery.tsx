...
import { NoContentFound } from "@platformx/utilities"
import './Gallery.css';
import { AUTH_INFO, ShowToastError, ThemeConstants } from '@platformx/utilities'; import { GalleryProps, ImageProps } from '../utils/galleryTypes';
;
...
const Gallery = ({
  ...
  const imageListProps = {
    ...
    size: 20,
    scope: AUTH_INFO.dspaceImagesUuid,
  };
  ...
  const videoListProps = {
    ...
    page: startIndex.current,
    size: 20,
    scope: AUTH_INFO.dspaceVideosUuid,
  };
  ...
  const handleSelectedImage = (selectedItem: ImageProps) => {
  ...
  const handleSelectedVideo = (selectedItem: ImageProps) => {
  ...
  const getData = (nextIndex: number) => {
    fetch(
      `${AUTH_INFO.dspaceUri}server/api/discover/search/objects?sort=${galleryMode === 'Images' ? imageListProps.sort : videoListProps.sort
      }&page=${nextIndex}&size=${galleryMode === 'Images' ? imageListProps.size : videoListProps.size
      }&scope=${galleryMode === 'Images' ? imageListProps.scope : videoListProps.scope
      }&dsoType=ITEM`
    )
      .then((res) => res.json())
      .then(
  ...
  const searchGallery = (index: number) => {
    ...
    if (galleryMode == 'Images') {
      fetch(
        `${AUTH_INFO.dspaceUri}server/api/discover/search/objects?sort=${imageSearchProps.sort}&page=${imageSearchProps.page}&size=${imageSearchProps.size}&configuration=${imageSearchProps.configuration}&query=${imageSearchProps.query}&scope=${imageSearchProps.scope}&embed=${imageSearchProps.embed}`
      )
        .then((res) => res.json())
        .then((json) => {
          if (json._embedded.searchResult._embedded.objects.length != 0) {
            const imageObjects = json._embedded.searchResult._embedded.objects;
            const thumbnailsArr: object[] = [];
            imageObjects.map((obj: { _embedded: { indexableObject: { _links: { bundles: { href: any; }; thumbnail: { href: any; }; }; metadata: { [x: string]: { value: any; }[]; }; }; }; }) => {
              thumbnailsArr.push({
                Bundles: obj._embedded.indexableObject._links.bundles.href,
                Thumbnail: obj._embedded.indexableObject._links.thumbnail.href,
                Title:
      ...
      fetch(
        `${AUTH_INFO.dspaceUri}server/api/discover/search/objects?sort=${videoSearchProps.sort}&page=${videoSearchProps.page}&size=${videoSearchProps.size}&configuration=${videoSearchProps.configuration}&query=${videoSearchProps.query}&scope=${videoSearchProps.scope}&embed=${videoSearchProps.embed}`
      )
        .then((res) => res.json())
        .then((json) => {
          if (json._embedded.searchResult._embedded.objects.length != 0) {
  ...
  const searchCategory = (index: number, searchText: string) => {
    const imageCategoryProps = {
      category: 'title',
      sort: `dc.${categoryRef.current == 'title' ? 'title' : 'contributor.author'
        },default,DESC`,
      page: index,
      size: 10,
    ...
    if (galleryMode == 'Images') {
      fetch(
        `${AUTH_INFO.dspaceUri}server/api/discover/browses/${imageCategoryProps.category}/items?scope=${imageCategoryProps.scope}&sort=${imageCategoryProps.sort}&page=${imageCategoryProps.page}&size=${imageCategoryProps.size}&startsWith=${imageCategoryProps.startsWith}`
      )
        .then((res) => res.json())
        .then((json) => {
      ...
      fetch(
        `${AUTH_INFO.dspaceUri}server/api/discover/browses/${videoCategoryProps.category}/items?scope=${videoCategoryProps.scope}&sort=${videoCategoryProps.sort}&page=${videoCategoryProps.page}&size=${videoCategoryProps.size}&startsWith=${videoCategoryProps.startsWith}`
      )
        .then((res) => res.json())
        .then((json) => {
          if (json._embedded.items.length != 0) {
