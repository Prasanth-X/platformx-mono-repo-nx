...
const FormValidate = () => {
  ...
  const emailValidation = (email = ""): any => {
    if (email) {
      return !emailValidate(email) ? `${t("email_id")} ${t("is_not_in_valid_format")}` : "";
    }
  ...
  const minimumErrorMsg = (name = "", value = "", min = 0) => {
    return value.length >= min ? "" : `${t(name)} ${t("need_minimum")} ${min} ${t("character")}`;
  };

  const maxErrorMsg = (name = "", value = "", max = 0) => {
    return max >= value.length ? "" : `${t(name)} ${t("allow_only")} ${max} ${t("character")}`;
  };
   ...
   * @returns string
   */
  const mandatoryValidateWithMinAndMax = ({ name = "", value = "", min = 0, max = 0 }): any => {
    let errorMsg = "";
    if (value) {
      if (max && min) {
        errorMsg =
