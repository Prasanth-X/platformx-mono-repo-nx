...
const conCatUrlPath = (ContentType = "") => {
  if (ContentType) {
    const typeUrl = ContentType.charAt(ContentType.length - 1) === "/" ? `${ContentType}` : `${ContentType}/`;
...
const BasicButton = (props: BasicButtonProps) => {
  ...
  const buttonClickEvent = async (e: any) => {
    ...
    if (buttonDataObj?.Button_Action === "Internal" || buttonDataObj?.Button_Action === "External") {
...
BasicButton.defaultProps = {
  ...
  secondaryArgs: {
    prelemBaseEndpoint: {
      PublishEndPoint: process.env.REACT_APP_PUBLISH_URI,
      language: "en",
    },
  },
};
