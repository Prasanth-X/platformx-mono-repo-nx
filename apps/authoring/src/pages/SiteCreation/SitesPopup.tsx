...
export default function SiteDialog(props) {
  ...
  const selectedSite =
    getSelectedSite() || storedSession?.userInfo?.default_site;
  const [site, setSite] = useState(selectedSite);
  const [filteredSites, setfilteredSites] = useState(
    storedSession?.userInfo?.accessible_sites.filter((a) => a != 'System')
  );

  const handleSearch = (value) => {
    setfilteredSites(
      storedSession?.userInfo?.accessible_sites
  ...
  const handleSiteChange = async (e) => {
    const isSiteSystem =
      e.target.textContent?.toLowerCase() === 'administration';
    try {
      const res = await axios.get(
        process.env.NX_API_URI +
          `auth/get-site-permissions/${e.target.textContent}`,
        {
          headers: {
      ...
      updateSession({
        ...
        role: res.data?.data?.roles?.find(
          (obj) => obj.site === res.data?.data?.selected_site
        )?.name,
      });
      const lang =
                ...
                <Box className={classes.container}>
                  ...
                  >
                    ...
                    <Box className={classes.sitescontent}>
                      <Typography
                        variant="h5medium"
                        className={classes.sitescontent}
                      >
                        {val}
                      </Typography>
