...
const AddQuestion = ({
  ...
  const handleChange = (event) => {
    ...
    if (
      ...
      if (answers.length > 2) {
        const temp = answers
          .filter((x, k) => x.option === '' && parseInt(k) > 1)
          .map((y) => y.id);
  ...
  const onSaveQuestion = (IsDuplicate = false) => {
    setExitPopUp(false);
    const { question, questionType, queBackgroundImg, backgroundColor } =
      addQuestionInfo;
    if (question === '') {
            ...
            >
              ...
              <Grid container>
                ...
                <Grid item xs={12} sm={7} md={7} className="textFiledLast">
                  ...
                  ) : addQuestionInfo.backgroundColor &&
                    !addQuestionInfo.isImg ? (
                    <Box
                      sx={{
                        width: '100%',
                        height: '206px',
