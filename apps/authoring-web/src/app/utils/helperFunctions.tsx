...
export const filterSelectedArticle = (state, articleName) => {
  const { article } = state;
  return article?.articleArray?.find((articleObj) => articleObj.Name === articleName);
};
    ...
    "@graph": [
      {
        ...
        description: description ? trimString(handleHtmlTags(description), 200) : "",
        embedUrl: videoUrl,
        thumbnailUrl: {
          "@id": thumbnailURL,
        },
...
export const getUniqueTimeZone = () => {
  ...
  const uniqueItems = data.filter(
    (item: any, index, self) => index === self.findIndex((x) => x.time === item.time),
  );
  return data;
};
...
export const formCroppedUrl = (url = "", ext = "") => {
  return `${authInfo.gcpUri}/${authInfo.gcpBucketName}/${url.replaceAll(" ", "%20")}.${ext}`;
};
